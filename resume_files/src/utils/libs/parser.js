...
function parseDictionaryRegular(data, Resume) {
  ...
  _.forEach(regularDictionary, function(expressions, key) {
    _.forEach(expressions, function(expression) {
      ...
      if (find && find.length && !find[0].toLowerCase().includes("curriculum") && !find[0].toLowerCase().includes("manager")) {
  ...
  if(!Resume.parts.name){
    ...
    if(firstname && lastname){
      ...
      for(let index = 0;index < d.length;index++) {
        if(regexName.test(d[index]) && !d[index].toLowerCase().includes("curriculum") && !d[index].toLowerCase().includes("manager") && d[index].split(" ").length <= 3){
